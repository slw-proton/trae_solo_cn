...
### 3.9 ⚖️ 关键设计取舍：不做社区检测（vs 微软 GraphRAG）

这是 LightRAG 与微软 **GraphRAG** 最本质的架构分歧，也是理解两者选型的关键。

#### GraphRAG 的做法：社区检测 + 层级摘要
...
| 步骤 | 操作 | 成本 |
|------|------|------|
| ① 实体/关系抽取 | 每个 chunk 调一次 LLM | 高 |
| ② Leiden 社区检测 | 对整个图做层次化聚类 | 中（纯计算） |
| ③ 社区摘要生成 | **每个社区调一次 LLM** 生成摘要文本 | **极高** |
| ④ 全局查询 | Global Search 遍历社区摘要树 | 中 |

**GraphRAG 的优势**：
- 全局理解能力极强——能回答"这本书的核心主题是什么"这类需要跨全文档归纳的问题
- 层级化结构天然支持"全局概览 → 逐步下钻"的探索模式

**GraphRAG 的代价**：
- **索引成本极高**：社区摘要是额外的 LLM 调用开销，大型数据集可能需要数百次额外 LLM 调用
- **增量更新几乎不可能**：新增文档后社区结构会变化，必须重新跑完整的 Leiden + 摘要流水线（全量重建）
- **延迟高**：Global Search 需要遍历多层社区摘要

#### LightRAG 的做法：扁平图谱 + 双层检索（无社区检测）
...
文档 → 分块 → LLM 抽取(实体+关系)
            ...
            ┌───────────┴───────────┐
            ↓                       ↓
    Local 检索 (实体向量)     Global 检索 (关系向量)
    "找相关实体→关联chunks"   "找相关关系→实体描述"
            └───────────┬───────────┘
                        ↓
                   Mix 融合 → LLM 生成
```

**LightRAG 故意放弃社区检测的原因**：

| 维度 | LightRAG（无社区） | GraphRAG（有社区） |
|------|-------------------|-------------------|
| **图谱结构** | 扁平实体-关系网络 | 层级化社区树 |
| **索引时 LLM 调用** | 仅抽取阶段 | 抽取 + **每个社区额外一次摘要** |
| **增量更新** | ✅ 原生支持 | ❌ 必须全量重建 |
| **全局问题能力** | ⚠️ 中等（靠 Global 检索 + 关系向量弥补） | ✅ 强（社区摘要提供天然的全局视角） |
| **索引速度** | 快 | 慢（多出社区摘要步骤） |
| **部署复杂度** | 低 | 高（需调优社区参数） |

#### 为什么 LightRAG 能用「双层检索」替代「社区检测」

LightRAG 的核心洞察是：**不需要社区摘要也能获得类似的全局理解能力**。

- **Local 检索** ≈ GraphRAG 的 Local Search（通过实体向量找具体细节）
- **Global 检索** ≈ GraphRAG 的 Global Search 的**轻量替代**（通过关系向量检索高层语义关联，而非遍历社区摘要树）

这种设计用**向量的语义相似度**替代了**图的结构化聚类**，换取了：
1. ✅ **增量更新成为可能**
2. ✅ **索引成本大幅降低**
3. ✅ **部署和运维更简单**

但代价也是真实的：
- ❌ 对于真正的"全局归纳型"问题（如"总结整本书的三大主题"），效果弱于 GraphRAG
- ❌ 缺乏层级化的知识组织，无法像 GraphRAG 那样自然地做"主题 → 子主题 → 实体"的下钻浏览

> **一句话总结**：LightRAG 用**双层向量检索**换来了**增量更新和轻量化**，牺牲了部分**全局归纳能力**。这是"效率 vs 深度"的经典工程取舍。
...
|------|-------------|---------------|------------|--------------|---------|
| **知识图谱** | ✅ 核心，双层检索（扁平图谱，**无社区检测**） | ❌ 无原生支持 | ⚠️ 基础 KG Index | ✅ **Leiden 社区检测 + 层级摘要** | ⚠️ 基础 |
| **增量更新** | ✅ 原生支持（无社区结构，合并简单） | N/A | ⚠️ 有限 | ❌ 社区变化需**全量重建** | ⚠️ 有限 |
| **全局归纳能力** | ⚠️ 中等（Global 检索弥补） | ❌ | ❌ | ✅ 极强（社区摘要驱动） | ❌ |
| **存储后端** | ✅ 12+ 种 | ✅ 多种向量库 | ✅ 多种 | ❌ 固定 Parquet 格式 | 较少 |
